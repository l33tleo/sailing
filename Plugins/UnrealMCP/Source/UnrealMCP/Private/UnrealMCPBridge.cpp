...
FString UUnrealMCPBridge::ExecuteCommand(const FString& CommandType, const TSharedPtr<FJsonObject>& Params)
{
    ...
    // Special handling for execute_python: use FTSTicker instead of AsyncTask
    // to avoid task graph recursion when Python calls ImportAssetTasks etc.
    if (CommandType == TEXT("execute_python"))
    {
        ...
        FTSTicker::GetCoreTicker().AddTicker(
            FTickerDelegate::CreateLambda([this, Params, Promise = MakeShared<TPromise<FString>>(MoveTemp(Promise))](float DeltaTime) -> bool
            {
                ...
                if (!Params->TryGetStringField(TEXT("code"), Code))
                {
                    ...
                    ResponseJson->SetStringField(TEXT("error"), TEXT("Missing 'code' parameter"));
                }
                else
                {
                    ...
                    {
                        ResponseJson->SetStringField(TEXT("status"), TEXT("error"));
                        ResponseJson->SetStringField(TEXT("error"), TEXT("Python plugin is not loaded. Enable the Python Editor Script Plugin."));
                    }
                    else
                    {
                        UE_LOG(LogTemp, Display, TEXT("UnrealMCPBridge: Executing Python via ticker (avoids task graph recursion)"));
                        ...
                        {
                            ResponseJson->SetStringField(TEXT("status"), TEXT("error"));
                            ResponseJson->SetStringField(TEXT("error"), TEXT("Python execution failed. Check Output Log."));
                        }
                    }
                }
    ...
    AsyncTask(ENamedThreads::GameThread, [this, CommandType, Params, Promise = MoveTemp(Promise)]() mutable
    {
        ...
        try
        {
            ...
            // Material Commands
                     ...
                     CommandType == TEXT("set_material_on_actor"))
            {
